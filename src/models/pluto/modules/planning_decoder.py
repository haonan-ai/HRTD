...
class ResDecoderLayer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio, dropout) -> None:
        super().__init__()
        self.dim = dim
    ...
    def forward(
        ...
        Args:
            tgt: (bs, R, N, K, dim)
            memory: (bs, M, dim)
            memory_key_padding_mask: (bs, M)

        Returns:
            tgt: (bs, R, N, K, dim)
        """
        bs, R, N, K, D = tgt.shape

        tgt = tgt.view(bs, R * N * K, D)
...
class PlanningDecoder(nn.Module):
    def __init__(
        ...
        self.res_decoder_blocks = nn.ModuleList(
            [
                ResDecoderLayer(dim, num_heads, mlp_ratio, dropout)
                for _ in range(decoder_depth)
            ]
        )
        ...
        self.res_emb = nn.Parameter(torch.Tensor(1, 1, 1, self.num_residual_modes, self.dim))
        ...
        self.res_loc_head = MLPLayer(self.dim, 2 * self.dim, self.future_steps * 2)
        self.res_yaw_head = MLPLayer(self.dim, 2 * self.dim, self.future_steps * 2)
        self.res_vel_head = MLPLayer(self.dim, 2 * self.dim, self.future_steps * 2)
        self.res_pi_head = MLPLayer(self.dim, self.dim, 1)

        nn.init.normal_(self.m_emb, mean=0.0, std=0.01)
        nn.init.normal_(self.m_pos, mean=0.0, std=0.01)
        nn.init.normal_(self.res_emb, mean=0.0, std=0.01)
    ...
    def forward(self, data, enc_data):
        ...
        if self.num_residual_modes > 0:
            q_res = self.res_proj(q).unsqueeze(3) + self.res_emb
            for blk in self.res_decoder_blocks:
                q_res = blk(
                    q_res,
                    enc_emb,
                    memory_key_padding_mask=enc_key_padding_mask,
                )
                assert torch.isfinite(q_res).all()
            res_loc = self.res_loc_head(q_res).view(bs, R, self.num_mode, self.num_residual_modes, self.future_steps, 2)
            res_yaw = self.res_yaw_head(q_res).view(bs, R, self.num_mode, self.num_residual_modes, self.future_steps, 2)
            res_vel = self.res_vel_head(q_res).view(bs, R, self.num_mode, self.num_residual_modes, self.future_steps, 2)
            res_pi = self.res_pi_head(q_res).squeeze(-1)
            res_traj = torch.cat([res_loc, res_yaw, res_vel], dim=-1)
            res_traj = traj.unsqueeze(3) + res_traj
            
            return traj, pi, res_traj, res_pi
        return traj, pi , None, None
