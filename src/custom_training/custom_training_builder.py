...
def build_training_engine(cfg: DictConfig, worker: WorkerPool) -> TrainingEngine:
    ...
    # Optionally load only model weights from a checkpoint (not full Lightning checkpoint)
    # This is intended for planner checkpoints like `checkpoints/pluto.ckpt` that contain
    # `state_dict` but are not 1:1 with the current LightningModuleWrapper.
    if cfg.get("model_checkpoint"):
        ckpt_path = Path(cfg.model_checkpoint)
        if not ckpt_path.is_file():
            logger.warning(f"model_checkpoint path does not exist: {ckpt_path}")
        else:
            logger.info(f"Loading model weights from model_checkpoint: {ckpt_path}")
            checkpoint = torch.load(ckpt_path, map_location="cpu")
            state_dict: Dict[str, torch.Tensor]
            if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
                state_dict = checkpoint["state_dict"]
            else:
                # Fallback: assume the whole object is a state_dict
                state_dict = checkpoint  # type: ignore[assignment]

            # Filter and adapt keys:
            # - LightningModuleWrapper expects keys like "model.xxx"
            # - Planner checkpoints may already have "model.xxx" keys
            model_state = model.state_dict()
            filtered_state_dict: Dict[str, torch.Tensor] = {}
            skipped_keys: List[Tuple[str, str]] = []

            for key, value in state_dict.items():
                # Keep Lightning-style "model." prefix; if missing, try to add it.
                candidate_keys = [key]
                if not key.startswith("model."):
                    candidate_keys.append(f"model.{key}")

                matched = False
                for ck in candidate_keys:
                    if ck in model_state:
                        if model_state[ck].shape == value.shape:
                            filtered_state_dict[ck] = value
                            matched = True
                        else:
                            skipped_keys.append(
                                (
                                    key,
                                    f"shape mismatch: checkpoint {value.shape} vs model {model_state[ck].shape} (mapped key: {ck})",
                                )
                            )
                        break

                if not matched and all(ck not in model_state for ck in candidate_keys):
                    skipped_keys.append(
                        (key, f"no matching key in current model (candidates: {candidate_keys})")
                    )

            missing, unexpected = model.load_state_dict(filtered_state_dict, strict=False)

            logger.info(
                f"Loaded {len(filtered_state_dict)} tensors from model_checkpoint; "
                f"{len(missing)} missing keys in current model; "
                f"{len(unexpected)} unexpected keys ignored from checkpoint; "
                f"{len(skipped_keys)} keys skipped due to shape or name mismatch."
            )
            if skipped_keys:
                logger.info("Details of skipped checkpoint keys:")
                for name, reason in skipped_keys:
