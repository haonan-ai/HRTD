...
class NuplanScenarioRender:
    ...
    def render(
        ...
        # 不再绘制 GT 车辆和 GT 轨迹，方便专注观察模型生成的三组轨迹
        # if gt_state is not None:
    ...
    def _plot_rollout_trajectories(self, ax, candidate_trajectories: np.ndarray):
        """
        支持两种可视化方式：
        - candidate_trajectories 形状为 (N, T, 3): 所有轨迹同一颜色（向后兼容）
        - candidate_trajectories 形状为 (G, K, T, 3): G 组轨迹，每组使用固定颜色区分
        """
        if candidate_trajectories.ndim == 3:
            # 兼容原有逻辑：全部用同一种颜色；这里也稍微加粗一点
            for traj in candidate_trajectories:
                ax.plot(
                    traj[:, 0],
                    traj[:, 1],
                ...
                ax.scatter(traj[-1, 0], traj[-1, 1], color="cyan", zorder=5, s=15)
            return

        if candidate_trajectories.ndim == 4:
            # 分组上色：例如 3 组，每组 4 条轨迹，分别用高饱和度颜色区分
            group_colors = ["red", "lime", "deepskyblue"]
            G = candidate_trajectories.shape[0]

            for g in range(G):
                color = group_colors[g % len(group_colors)]
                for traj in candidate_trajectories[g]:
                    ax.plot(
                        traj[:, 0],
                        traj[:, 1],
                        alpha=1.0,
    ...
    def _plot_prediction(self, ax, predictions: np.ndarray):
        # 其他 agent 的预测轨迹：画细一点，避免干扰主观察的三组轨迹
        kwargs = {"lw": 1.0}
